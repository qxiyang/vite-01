    ...
    <div class="container">
        ...
        <van-cell-group class="cell">
            ...
            <van-cell>
              ...
              <template #default>
                  <div @click="showHosptialTime=true">
                      {{currentDate || "请选择就诊时间"}}
                      <van-icon name="arrow" />
                  </div>
              </template>                
        ...
        <van-cell-group title="服务内容" class="cell">
          <van-field 
            class="text" 
            style="size: 100px;"
            v-model="form.demand" 
        ...
        <van-button class="sumbit" type="primary" size="large" @click="submit">提交</van-button>
        <van-popup
        ...
        <van-popup
          v-model:show="showHosptialTime"
          position="bottom"
          :style="{ height: '30%' }"
        >
          <van-date-picker
            title="选择日期"
            @confirm="showHosptialTimeConfirm"
            @cancel="onCancel"
            :min-date="minDate"
          />
        </van-popup>
...
//就诊时间
const showHosptialTime = ref(false)
const currentDate = ref()
//选择就诊时间
const showHosptialTimeConfirm = (item) => {
    //获取字符时间 
    const dateStr = item.selectedValues.join('-')
    currentDate.value = dateStr
    //存储数据调整类型为后端要求的number
    form.starttime = new Date(dateStr).getTime()
    //console.log(typeof(form.starttime))//number
    showHosptialTime.value = false
}
//最小时间为当前日期
